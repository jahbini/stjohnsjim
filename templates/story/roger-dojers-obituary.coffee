# #-------- class start
renderer = class  roger_dojers_obituary extends stjohnsjimtemplate
  # 
  # section html
  # 
  # 
  ...
  mainUnused: =>
    T.div "#main", =>
      @post_roger_dojers_obituary()
  # 
  # section post_roger_dojers_obituary
  # 
  post_roger_dojers_obituary: =>
    T.div "#post-roger-dojers-obituary.article.article-type-post", itemscope: "itemscope", itemprop: "blogPost", =>
      T.div ".article-inner.pb2", =>
        T.div ".article-header", =>
          T.h1 ".article-title", itemprop: "name", => T.raw "Roger Dojer's Obituary"
          @bloviation()
  ...
  bloviation: =>
    T.div "#bloviation.article-entry.contents", "dangerously-set-inner-h-t-m-l": "[object Object]", =>
      T.p =>
        T.raw "Roger Dojer, Born 2010, Died 2011."
        T.br()
        T.raw "Solid as a Rock"
      ...
      T.p => T.raw "After finding Tommy’s ear-prints on his door, Roger decided to get a second Ph.D. in Minding Your Own Business from an on-line University."
      T.p => T.raw "One day, when the supply of toilet paper ran out, Tommy used Roger’s good name instead."
      ...
      T.p =>
        T.raw "Roger left for life in Coos Bay to complete his Ph.D. in MYOB and pursue a life long passion to pretend to sing —"
        T.span ".FBname", => T.raw "Friend"
        T.raw ", It’s something the really, really drunk do: It’s like vocal beer glasses. If you drink enough, you sing better and better. It’s magic, try it,"
        T.span ".FBname", => T.raw "Friend"
        T.raw ". You can even end up in the morning with some stupid song ringing in your head in time to the throb in your eyes accompanied by that corn cob rasp in your throat. Thus is born Karaoke."
      T.p => T.raw "The highest accolade in Karaoke is to sing “I did it Myyyyyyyyyyyyyyy way” horribly off-key and actually make it through the night alive. Roger was too smart to actually sing that song in public, but his private goal was to affirm his life spirit through alcohol enhanced song."
      ...
      T.p => T.raw "The evidence is clear. The Great Pacific Northwest Fungal Mat has more power as a critic than Roger Ebert. Don’t be the next victim."
      T.p => T.raw "Roger’s hobbies included finding a woman his age who was not in an iron lung and riding his beloved man-toy vehicles. Roger is survived by everyone."
      T.p => T.raw "R.I.P. Roger Dojer. We have lost stability and replaced it with … Um. Replaced with … Um."
      T.p =>
        T.span ".FBname", => T.raw "Friend"
        T.raw ", I’ll get back to you with that."
  # 
  # section header
  # 
  # 
