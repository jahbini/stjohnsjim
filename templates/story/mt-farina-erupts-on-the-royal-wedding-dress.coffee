# 
class mt_farina_erupts_on_the_royal_wedding_dress extends stjohnsjimtemplate
  # 
  # section html
  # 
  # 
  ...
  main: =>
    T.div "#main", =>
      @post_mt_farina_erupts_on_the_royal_wedding_dress()
  # 
  # section post_mt_farina_erupts_on_the_royal_wedding_dress
  # 
  post_mt_farina_erupts_on_the_royal_wedding_dress: =>
    T.div "#post-mt-farina-erupts-on-the-royal-wedding-dress.article.article-type-post", itemscope: "itemscope", itemprop: "blogPost", =>
      T.div ".article-inner.pb2", =>
        T.div ".article-header", =>
          T.h1 ".article-title", itemprop: "name", => T.raw "Mt. Farina erupts on the Royal Wedding Dress"
          @bloviation()
  ...
  bloviation: =>
    T.div "#bloviation.article-entry.contents", "dangerously-set-inner-h-t-m-l": "[object Object]", =>
      ...
      T.p =>
        ...
        T.em => T.raw "Legendary"
        T.raw "day that marks, well, essentially nothing."
      ...
      T.p => T.raw "A spokesperson on the tube actually says it is the British taxpayers way of participating and they have the opportunity to be thankful for being allowed to participate."
      T.p =>
        T.raw "TV Announcer: did you love the dress?"
        T.br()
        T.raw "Attending street crowd cheers wildly, drowning out announcer for 15 seconds."
        T.br()
        T.raw "Announcer: And what about her stunning hair?"
        T.br()
        T.raw "Again, the crowd cheers wildly."
      T.p => T.raw "The blaring TV cuts away to an advertisement for a weight loss program. I realize that every woman watching wants to be the bride in a fairy tale wedding: At least we have a real woman as a public model of femininity. The Royal Weddings have produced several stand-up women, and that&rsquo;s pretty good."
      ...
      T.p => T.raw "The company only published directions for Microwave cooking. They did not give directions for cleaning the resulting disaster. Boiling mush powered by rogue microwave energy will erupt and stick to the bowl, the oven, the sink, everything, even the show on TV. &nbsp;The bride didn’t even have time to duck."
      ...
      T.p =>
        T.raw "The royals hardly noticed the interruption. &nbsp;I&rsquo;m just glad that"
        T.a href: "http://www.imdb.com/title/tt0120789/", => T.raw "Don Knotts"
        T.raw "is my TV fix-it guy. I did only have seven seconds, and I think the British Taxpayer has the opportunity to be happy that I fixed it before everyone saw goo on the bride&rsquo;s dress. Bill Clinton would have been pilloried again."
      T.p => T.raw "Bill, you owe me one. This Wedding Fairy Tale is on me.&nbsp;"
  # 
  # section header
  # 
  # 
...
page = new mt_farina_erupts_on_the_royal_wedding_dress
